...
class DataGenerator():
    def __init__(self, DataFileName=None, GTFileName=None, hwz=3, CL_num=52, NL_num=8, dim=3):  # NL_num=4
    ...
    def pca_transform(self, data, nc=4, verbosity=False):
    ...
    def FormClassDict(self):
        ...
        temp = scale(temp)
        # temp = self.pca_transform(temp, nc = self.dim) #add
        self.in_channel = temp.shape[-1]
        info = temp.reshape((self.data.shape[0], self.data.shape[1], -1))
        # Data padding
        paddingData = np.zeros(
            (info.shape[0] + 2 * self.hwz + 1, info.shape[1] + 2 * self.hwz + 1, info.shape[2]))
        paddingData[self.hwz: info.shape[0] + self.hwz,
                    self.hwz: info.shape[1] + self.hwz] = info[:, :]
        ...
        for x in range(self.groundtruth.shape[0]):
            for y in range(self.groundtruth.shape[1]):
                ...
                self.ClassDict[self.groundtruth[x, y]].append(
                    paddingData[x: x + 2 * self.hwz + 1, y: y + 2 * self.hwz + 1, :].transpose(2, 0, 1).astype(np.float32))
        return
    ...
    def GenerateTrainingSet(self, CL_NUM, NL_NUM):
        ...
        for key in self.ClassDict.keys():
            ...
            # samples = random.sample(self.ClassDict[key], min(CL_NUM, len(self.ClassDict[key]))) #indian_pines
            for i in range(CL_NUM):
                self.training.append((samples[i], key - 1, key - 1))
                self.ClassDict[key] = self.RemoveFromList(
                    self.ClassDict[key], samples[i])
            for j in range(NL_NUM):
                ...
                self.ClassDict[truth] = self.RemoveFromList(
                    self.ClassDict[truth], true_sample)
        return
